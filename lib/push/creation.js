...
import { throttleLet } from 'rx-op-lossless-throttle'
...
// const rateLimiter = new RateLimiter(REQUESTS_PER_SECOND, 1000)
...
export function createEntities (context, entities, destinationEntities) {
  return Rx.Observable.zip(
    Rx.Observable.from(entities)
      .bufferTime(1000, null, REQUESTS_PER_SECOND)
      .concatMap(buffer => Rx.Observable
        .from(buffer)
        .mergeMap((entity) => {
          ...
          const action = Rx.Observable.defer(() => {
          ...
          return action
            .retryWhen((errors) => errors
              .mergeMap((error) => {
                if (error.status === 429) {
                  const wait = Math.floor(error.headers['x-contentful-ratelimit-reset'] * 1000 + (Math.random() * 200))
                  return Rx.Observable.timer(wait)
                }
                return Rx.Observable.throw(error)
              })
            )
            .do(partial(creationSuccessNotifier, operation))
            .catch((err) => Rx.Observable.of(handleCreationErrors(entity, err)))
        })
        .delay(1000) // @todo add smart delay here
      )
  )
    .scan((a, c) => a.concat(c), [])
    .toPromise()
